#!/usr/bin/env python3
"""
Call of Cthulhu 7e Character Sheets — v2 REDESIGN
Event 92 — Last Train to Coney Island (NYC, 1979)
Theme: 1970s/80s urban — Anton display type, condensed sans, MTA red-black
ChaosiumCon 2026
"""
...
# ── FONTS ─────────────────────────────────────────────────────────────────────
pdfmetrics.registerFont(TTFont('Anton',       '/tmp/anton.ttf'))
pdfmetrics.registerFont(TTFont('DJVSCondBold','/usr/share/fonts/truetype/dejavu/DejaVuSansCondensed-Bold.ttf'))
pdfmetrics.registerFont(TTFont('DJVSCond',    '/usr/share/fonts/truetype/dejavu/DejaVuSansCondensed.ttf'))
pdfmetrics.registerFont(TTFont('LibSansBold', '/usr/share/fonts/truetype/liberation/LiberationSans-Bold.ttf'))
pdfmetrics.registerFont(TTFont('LibSansReg',  '/usr/share/fonts/truetype/liberation/LiberationSans-Regular.ttf'))
pdfmetrics.registerFont(TTFont('STMono',      '/tmp/sharetechmono.ttf'))
...
# ═══════════════════════════════════════════════════════════════════════════════
# THEME — 1979 NYC: cool newsprint · jet black · MTA red · stark contrast
# No serifs. No warmth. No parchment. The city is broke, burning, and on the edge.
# ═══════════════════════════════════════════════════════════════════════════════
...
def last_train_theme():
    ...
    t.name = 'ltci'
    # ── Page ──
    t.page_bg         = colors.HexColor('#EDEAE0')   # cool newsprint — not warm
    t.parchment       = colors.HexColor('#EDEAE0')
    t.parchment_dark  = colors.HexColor('#DEDAD0')
    # ── Header (jet black with red spine) ──
    t.header_bg       = colors.HexColor('#080808')
    t.header_inner    = colors.HexColor('#101010')
    t.header_border   = colors.HexColor('#CC2800')   # MTA red
    t.header_name     = colors.HexColor('#F5F2E8')   # stark white-cream
    t.header_sub      = colors.HexColor('#FF6030')   # hot orange on black
    t.header_right    = colors.HexColor('#A8A090')
    # ── Section banners (jet black) ──
    t.banner_bg       = colors.HexColor('#0A0A08')
    t.banner_text     = colors.HexColor('#F5F2E8')
    t.banner_special  = colors.HexColor('#1A0806')
    # ── Characteristics ──
    t.stat_hdr_bg     = colors.HexColor('#0A0A08')
    t.stat_hdr_text   = colors.HexColor('#FF6030')   # hot orange on black
    t.stat_cell_bg    = colors.HexColor('#DEDAD0')
    t.stat_cell_text  = colors.HexColor('#080808')
    # ── Derived stats strip ──
    t.derived_bg      = colors.HexColor('#0A0A08')
    t.derived_label   = colors.HexColor('#FF6030')
    t.derived_val     = colors.HexColor('#F5F2E8')
    # ── Skill / equipment rows ──
    t.row1            = colors.HexColor('#EDEAE0')
    t.row2            = colors.HexColor('#DEDAD0')
    t.row_text        = colors.HexColor('#080808')
    t.rule            = colors.HexColor('#282820')   # near black rule
    # ── Brief box ──
    t.special_bg      = colors.HexColor('#080808')
    t.special_border  = colors.HexColor('#CC2800')
    t.special_title   = colors.HexColor('#FF6030')
    t.special_label   = colors.HexColor('#CC4010')
    t.special_body    = colors.HexColor('#E8E4D4')
    # ── HP track (blood red) ──
    t.hp_label        = colors.HexColor('#7A0000')
    t.hp_max          = colors.HexColor('#7A0000')
    t.hp_box_fill     = colors.HexColor('#FFF0EC')
    t.hp_box_5th      = colors.HexColor('#FFBBA8')
    t.hp_box_border   = colors.HexColor('#AA1800')
    t.hp_num          = colors.HexColor('#3A0000')
    # ── SAN track (concrete green — subway tile) ──
    t.san_label       = colors.HexColor('#0A2A18')
    t.san_box_fill    = colors.HexColor('#E4F2EC')
    t.san_box_5th     = colors.HexColor('#A0D8B8')
    t.san_box_border  = colors.HexColor('#1A6040')
    t.san_num         = colors.HexColor('#082010')
    # ── Luck ──
    t.luck_label      = colors.HexColor('#0A1850')
    t.luck_box_fill   = colors.HexColor('#E8EEFF')
    t.luck_box_5th    = colors.HexColor('#AABCEC')
    t.luck_box_border = colors.HexColor('#1A2888')
    t.luck_num        = colors.HexColor('#080F38')
    # ── Portrait placeholder ──
    t.port_bg         = colors.HexColor('#0A0A08')
    t.port_border     = colors.HexColor('#CC2800')
    t.port_text       = colors.HexColor('#303028')
    t.port_label      = colors.HexColor('#FF6030')
    # ── Personal hook box ──
    t.hook_bg         = colors.HexColor('#F8F5EC')
    t.hook_border     = colors.HexColor('#CC2800')
    t.hook_bar        = colors.HexColor('#CC2800')
    t.hook_label      = colors.HexColor('#6A0800')
    t.hook_body       = colors.HexColor('#080808')
    # ── Back header ──
    t.back_bg         = colors.HexColor('#080808')
    t.back_border     = colors.HexColor('#CC2800')
    t.back_sub        = colors.HexColor('#FF6030')
    # ── Body ──
    t.body            = colors.HexColor('#080808')
    t.italic          = colors.HexColor('#303028')
    t.quote           = colors.HexColor('#6A0800')
    # ── Cover ──
    t.cover_row1      = colors.HexColor('#0A0A08')
    t.cover_row2      = colors.HexColor('#181816')
    t.cover_name      = colors.HexColor('#F5F2E8')
    t.cover_arch      = colors.HexColor('#FF6030')
    t.cover_meta      = colors.HexColor('#909080')
    t.cover_note      = colors.HexColor('#909080')
    t.cover_title     = colors.HexColor('#F5F2E8')   # white Anton on red band
    t.cover_sub       = colors.HexColor('#F5F2E8')
    t.cover_byline    = colors.HexColor('#606050')
    t.cover_rule_body = colors.HexColor('#F5F2E8')
    t.accent          = colors.HexColor('#CC2800')   # MTA red
    t.accent2         = colors.HexColor('#FF6030')   # hot orange
    t.footer          = colors.HexColor('#484840')
    ...
    # ── Fonts ──
    t.font_head       = 'Anton'         # display — character names, titles
    t.font_subhead    = 'DJVSCondBold'  # section headers, labels
    t.font_body       = 'LibSansReg'    # body text
    t.font_body_bold  = 'LibSansBold'   # bold body
    return t
...
def make_styles(t):
    fh  = t.font_head       # Anton
    fsh = t.font_subhead    # DJVSCondBold
    fb  = t.font_body       # LibSansReg
    fbb = t.font_body_bold  # LibSansBold
    return {
        'cover_title': ParagraphStyle('ct', fontName=fh,  fontSize=30, textColor=t.cover_title,  alignment=TA_LEFT,   leading=36, spaceAfter=4),
        'cover_sub':   ParagraphStyle('cs', fontName=fsh, fontSize=11, textColor=t.cover_sub,    alignment=TA_LEFT,   leading=15, spaceAfter=2),
        'cover_byline':ParagraphStyle('cb', fontName=fb,  fontSize=8,  textColor=t.cover_byline, alignment=TA_LEFT,   leading=12),
        'cover_name':  ParagraphStyle('cn', fontName=fsh, fontSize=11, textColor=t.cover_name,   leading=15),
        'cover_arch':  ParagraphStyle('ca', fontName=fh,  fontSize=10, textColor=t.cover_arch,   leading=14),
        'cover_meta':  ParagraphStyle('cm', fontName=fb,  fontSize=8,  textColor=t.cover_meta,   leading=12),
        'cover_rule':  ParagraphStyle('cr', fontName=fbb, fontSize=8,  textColor=t.cover_rule_body, alignment=TA_CENTER, leading=11),
        'cover_note':  ParagraphStyle('co', fontName=fb,  fontSize=9,  textColor=t.cover_note, alignment=TA_CENTER, leading=13, fontStyle='italic' if hasattr(t,'x') else ''),
        'rules_head':  ParagraphStyle('rh', fontName=fsh, fontSize=10, textColor=t.accent2,      alignment=TA_CENTER, leading=14, spaceBefore=4, spaceAfter=3),
        'body':        ParagraphStyle('b',  fontName=fb,  fontSize=8.5,textColor=t.body,         leading=12, spaceAfter=3, alignment=TA_JUSTIFY),
        'body_sm':     ParagraphStyle('bs', fontName=fb,  fontSize=7.5,textColor=t.row_text,     leading=10, spaceAfter=0),
        'body_sm_bold':ParagraphStyle('bb', fontName=fbb, fontSize=7.5,textColor=t.row_text,     leading=10, spaceAfter=0),
        'sp_title':    ParagraphStyle('st', fontName=fsh, fontSize=11, textColor=t.special_title, leading=14, spaceBefore=1),
        'sp_label':    ParagraphStyle('sl', fontName=fbb, fontSize=7,  textColor=t.special_label, leading=10, spaceAfter=1),
        'sp_body':     ParagraphStyle('sb', fontName=fb,  fontSize=8,  textColor=t.special_body,  leading=11, spaceAfter=2, alignment=TA_JUSTIFY),
        'hook_label':  ParagraphStyle('hl', fontName=fsh, fontSize=9,  textColor=t.hook_label,   leading=12),
        'hook_body':   ParagraphStyle('hb', fontName=fb,  fontSize=8,  textColor=t.hook_body,    leading=12, alignment=TA_JUSTIFY),
        'quote':       ParagraphStyle('q',  fontName=fh,  fontSize=9,  textColor=t.quote,        alignment=TA_CENTER, leading=13, spaceBefore=3),
        'italic_sm':   ParagraphStyle('is', fontName=fb,  fontSize=8,  textColor=t.italic,       leading=11, alignment=TA_JUSTIFY),
        'notes_head':  ParagraphStyle('nh', fontName=fsh, fontSize=9,  textColor=t.banner_text,  leading=13),
        'wt_hdr':      ParagraphStyle('wth',fontName=fbb, fontSize=7,  textColor=colors.white,   leading=9),
        'wt_body':     ParagraphStyle('wtb',fontName=fb,  fontSize=7,  textColor=t.row_text,     leading=9),
        'wt_body_c':   ParagraphStyle('wtc',fontName=fbb, fontSize=7,  textColor=t.row_text,     leading=9, alignment=TA_CENTER),
        'wt_dodge':    ParagraphStyle('wtd',fontName=fbb, fontSize=8,  textColor=t.accent2,      leading=11),
        'ref_title':   ParagraphStyle('rft',fontName=fh,  fontSize=18, textColor=t.accent2,      alignment=TA_CENTER, leading=22, spaceAfter=2),
        'ref_sub':     ParagraphStyle('rfs',fontName=fb,  fontSize=8.5,textColor=t.italic,       alignment=TA_CENTER, leading=12),
        'ref_hdr':     ParagraphStyle('rfh',fontName=fbb, fontSize=7,  textColor=colors.white,   leading=9),
        'ref_body':    ParagraphStyle('rfb',fontName=fb,  fontSize=7.5,textColor=t.body,         leading=10),
        'ref_bold':    ParagraphStyle('rfc',fontName=fbb, fontSize=7.5,textColor=t.body,         leading=10),
        'ref_note':    ParagraphStyle('rfn',fontName=fb,  fontSize=7.5,textColor=t.italic,       alignment=TA_CENTER, leading=10),
    }


# ═══════════════════════════════════════════════════════════════════════════════
# FLOWABLES — redesigned for sharp 70s/80s urban aesthetic
# ═══════════════════════════════════════════════════════════════════════════════

class OrnRule(Flowable):
    """Double rule — thick over thin. Clean 70s graphic design, no ornaments."""
    def __init__(self, width, t):
        super().__init__(); self._t=t; self.width=width; self.height=10
    def wrap(self,a,b): return (self.width,self.height)
    def draw(self):
        c=self.canv; t=self._t
        c.setStrokeColor(t.accent); c.setLineWidth(2.5)
...
class CharHeader(Flowable):
    """Character header: black band, bold left red spine, Anton name, clean info."""
    def __init__(self, name, arch, meta, allg, width, t):
        super().__init__(); self._n=name; self._a=arch; self._m=meta; self._g=allg
        self.width=width; self.height=54; self._t=t
    def wrap(self,a,b): return (self.width,self.height)
    def draw(self):
        c=self.canv; t=self._t; w,h=self.width,self.height
        # Black background (sharp corners — no rounded rect)
        c.setFillColor(t.header_bg); c.rect(0,0,w,h,fill=1,stroke=0)
        # Red left spine bar
        c.setFillColor(t.accent); c.rect(0,0,6,h,fill=1,stroke=0)
        # Bottom thin accent line
        c.setStrokeColor(t.accent); c.setLineWidth(0.8)
        c.line(6,2,w,2)
        # Character name in Anton — large, all-caps naturally
        c.setFillColor(t.header_name); c.setFont(t.font_head,22)
        c.drawString(14,h-26,self._n.upper())
        # Role · setting on second line
        c.setFillColor(t.header_sub); c.setFont(t.font_subhead,8.5)
        c.drawString(14,h-40,f"{self._a}  ·  {self._m}")
        # Connection/hook right-aligned
        c.setFillColor(t.header_right); c.setFont(t.font_body,7.5)
        c.drawRightString(w-8,h-40,self._g)
        # Small red rectangle accent top-right
        c.setFillColor(t.accent); c.rect(w-12,h-12,10,10,fill=1,stroke=0)
...
class StatBlock(Flowable):
    ...
    def draw(self):
        ...
        c.setStrokeColor(t.rule); c.setLineWidth(0.4); c.rect(0,0,w,13,fill=0,stroke=1)
        x_pos=8
        for lbl,val in self._d:
            c.setFillColor(t.derived_label); c.setFont(t.font_subhead,6.5)
            c.drawString(x_pos,3,lbl+':')
            lw=c.stringWidth(lbl+':',t.font_subhead,6.5)
            c.setFillColor(t.derived_val); c.setFont(t.font_body_bold,7.5)
            c.drawString(x_pos+lw+2,2.5,str(val))
            vw=c.stringWidth(str(val),t.font_body_bold,7.5)
            x_pos+=lw+vw+10


class SectionBanner(Flowable):
    """Section banner: black band, red left pip, condensed bold sans label."""
    def __init__(self, text, width, t, special=False):
        super().__init__(); self._text=text; self.width=width; self._t=t
        self._sp=special; self.height=16
    def wrap(self,a,b): return (self.width,self.height)
    def draw(self):
        c=self.canv; t=self._t
        ...
        c.setFillColor(t.accent); c.rect(0,0,4,16,fill=1,stroke=0)
        # Label — condensed bold sans
        c.setFillColor(t.banner_text); c.setFont(t.font_subhead,9)
        c.drawString(12,4,self._text.upper())


class HPTrack(Flowable):
    """Clean HP/SAN track — square boxes, sharp borders, condensed labels."""
    BOX=12; GAP=3
    ...
    def draw(self):
        ...
        for i in range(max_val):
            ...
            val=max_val-i
            # Shadow (subtle)
            c.setFillColor(colors.HexColor('#C0BDB0')); c.rect(x+1,y-1,B,B,fill=1,stroke=0)
            # Box — square corners
            fill=self._fill5 if (val%5==0) else self._fill
            c.setFillColor(fill); c.setStrokeColor(self._border); c.setLineWidth(0.8)
            c.rect(x,y,B,B,fill=1,stroke=1)
            # Number
            c.setFillColor(self._num); c.setFont(self._t.font_body_bold,8)
...
class PortraitPlaceholder(Flowable):
    ...
    def draw(self):
        ...
        c.setFillColor(t.port_bg); c.rect(0,0,w,h,fill=1,stroke=0)
        c.setStrokeColor(t.port_border); c.setLineWidth(1.2); c.rect(0,0,w,h,fill=0,stroke=1)
        # Grid lines
        c.setStrokeColor(t.port_text); c.setLineWidth(0.2)
        ...
        c.setFillColor(t.port_label); c.setFont(t.font_subhead,8.5)
        c.drawCentredString(w/2,mid_y+14,"PORTRAIT RESERVED")
        c.setFillColor(t.port_text); c.setFont(t.font_body,7)
        c.drawCentredString(w/2,mid_y+2,self._name)
        c.setFont(t.font_body,6.5); c.drawCentredString(w/2,mid_y-10,"Midjourney art")
        # Corner squares
        c.setStrokeColor(t.port_border); c.setLineWidth(1.2); sz=8
...
class BackHeader(Flowable):
    ...
    def draw(self):
        c=self.canv; t=self._t; w,h=self.width,self.height
        c.setFillColor(t.back_bg); c.rect(0,0,w,h,fill=1,stroke=0)
        c.setFillColor(t.accent); c.rect(0,0,6,h,fill=1,stroke=0)
        c.setStrokeColor(t.back_border); c.setLineWidth(0.6); c.rect(0,0,w,h,fill=0,stroke=1)
        c.setFillColor(t.header_name); c.setFont(t.font_head,14)
        c.drawString(12,h-19,self._n.upper())
        nw=c.stringWidth(self._n.upper(),t.font_head,14)
        c.setFillColor(t.back_sub); c.setFont(t.font_body,7.5)
        c.drawString(16+nw,h-18,f"·  {self._a}  ·  {t.back_note}")
...
# ═══════════════════════════════════════════════════════════════════════════════
# PAGE BACKGROUNDS — sharp urban grid, not parchment
# ═══════════════════════════════════════════════════════════════════════════════
...
def make_cover_bg(t):
    def fn(c, doc):
        ...
        # Subtle grid (faint, like graph paper under a police bulletin)
        c.setStrokeColor(colors.HexColor('#141410')); c.setLineWidth(0.2)
        for y in range(0,int(h),20): c.line(0,y,w,y)
        for x in range(0,int(w),20): c.line(x,0,x,h)
        # BOLD red horizontal title band — takes up top third
        c.setFillColor(t.accent); c.rect(0,h-110,w,90,fill=1,stroke=0)
        # Thin bright accent line at top of red band
        c.setFillColor(colors.HexColor('#FF4010')); c.rect(0,h-22,w,4,fill=1,stroke=0)
        # Dark footer band
        c.setFillColor(colors.HexColor('#101010')); c.rect(0,0,w,52,fill=1,stroke=0)
        # Red spine bar on left edge — full height
        c.setFillColor(colors.HexColor('#9A1800')); c.rect(0,0,8,h,fill=1,stroke=0)
        c.restoreState()
    return fn
...
def make_page_bg(t):
    def fn(c, doc):
        ...
        # Very faint grid
        c.setStrokeColor(colors.HexColor('#D8D5C8')); c.setLineWidth(0.15)
        for y in range(0,int(h),12): c.line(0,y,w,y)
        # Bold outer border — near black, sharp corners
        c.setStrokeColor(colors.HexColor('#181816')); c.setLineWidth(2.0)
        c.rect(8,8,w-16,h-16,fill=0,stroke=1)
        # Thin red inner border
        c.setStrokeColor(t.accent); c.setLineWidth(0.5)
        c.rect(11,11,w-22,h-22,fill=0,stroke=1)
        # Red corner squares (not diamonds)
        c.setFillColor(t.accent)
        for cx,cy in [(8,8),(w-8,8),(8,h-8),(w-8,h-8)]:
            c.rect(cx-4,cy-4,8,8,fill=1,stroke=0)
        # Footer
        c.setFont(t.font_body,7); c.setFillColor(t.footer)
