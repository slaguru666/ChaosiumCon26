...
def draw_front(c, char, page_num, total_pages):
    ...
    stats_h = 26*mm
    filled_rect(c, 0, stats_y - stats_h, PW, stats_h, PALE)
    ...
    for i, (lbl, val) in enumerate(stats):
        ...
        # value
        text(c, str(val), bx + bw/2, by + 4, 'OrbB', 23, INK, align='centre')

    # ── DERIVED STATS ROW ─────────────────────────────────────────────────────
    der_y = stats_y - stats_h
    der_h = 12*mm
    filled_rect(c, 0, der_y - der_h, PW, der_h, HexColor('#eeeef8'))
    ...
    hp_y = der_y - der_h
    hp_sec_h = 10*mm
    filled_rect(c, 0, hp_y - hp_sec_h, PW, hp_sec_h, white)
    ...
    text(c, f'UNCONSCIOUS ≤ {unc}  ·  DYING at 0  ·  DEAD at −{con}',
         PW - PAD, hp_y - hp_sec_h + 3*mm, 'Mono', 5.5, HexColor('#444466'), align='right')
    ...
    # Weapons — proper 4-column layout: NAME | SKILL% | DAMAGE | RANGE + notes
    # Column x positions (relative to PAD)
    uw = PW - 2*PAD
    c_name  = PAD                    # weapon name  — 38% of uw
    c_skill = PAD + uw*0.38          # skill%       — 14%
    c_dmg   = PAD + uw*0.52          # damage       — 25%
    c_range = PAD + uw*0.77          # range        — 23% (right-anchored)

    # Column headers
    hdr_y = wpn_y + wpn_h - 20
    HDR_COL = HexColor('#555566')
    text(c, 'WEAPON NAME',  c_name+2,  hdr_y, 'OrbB', 7, HDR_COL)
    text(c, 'SKILL %',      c_skill+2, hdr_y, 'OrbB', 7, HDR_COL)
    text(c, 'DAMAGE',       c_dmg+2,   hdr_y, 'OrbB', 7, HDR_COL)
    text(c, 'RANGE',        c_range+2, hdr_y, 'OrbB', 7, HDR_COL)
    hline(c, PAD, PW-PAD, hdr_y - 2, MID, lw=0.6)
    # Thin column dividers in header
    for cx in [c_skill, c_dmg, c_range]:
        hline(c, cx, cx, hdr_y+7, HexColor('#ccccdd'), lw=0.3)
    ...
    for wi, wpn in enumerate(char['weapons']):
        ...
        for cx in [c_skill, c_dmg, c_range]:
            hline(c, cx-1, cx-1, row_y+9*mm, HexColor('#d8d8ec'), lw=0.4)
            c.saveState(); c.setStrokeColor(HexColor('#d8d8ec')); c.setLineWidth(0.4)
            c.line(cx-1, row_y-1*mm, cx-1, row_y+9*mm)
            c.restoreState()

        # Col 1: Weapon name (bold) + notes sub-line
        if len(wpn) > 0:
            text(c, str(wpn[0]), c_name+3, row_y+5.5*mm, 'Bar', 10, INK)
        if len(wpn) > 4 and wpn[4]:
            text(c, str(wpn[4]), c_name+3, row_y+1.8*mm, 'Mono', 6.5,
                 HexColor('#555566'))

        # Col 2: Skill % — large accent
        if len(wpn) > 1:
        ...
        if len(wpn) > 3:
            rng = str(wpn[3])
            text(c, rng, c_range+3, row_y+4.5*mm, 'Mono', 8,
                 HexColor('#333355'))
    ...
    # Equipment (left)
    filled_rect(c, 0, notes_y, half_w + PAD + 1.5, notes_h, white)
    outline_rect(c, 0, notes_y, half_w + PAD + 1.5, notes_h, A, lw=1.2)
    text(c, 'EQUIPMENT', PAD, notes_y + notes_h - 7, 'OrbB', 7.5, A)
    eq_lines = char['equip']
    ey = notes_y + notes_h - 16
    for eq in eq_lines:
        text(c, '▸ ' + eq, PAD, ey, 'Bar', 8, INK)
        ey -= 9

    # Personality (right)
    rx = half_w + PAD + 3
    filled_rect(c, rx, notes_y, PW - rx, notes_h, white)
    outline_rect(c, rx, notes_y, PW - rx, notes_h, A, lw=1.2)
    text(c, 'CHARACTER', rx + 3, notes_y + notes_h - 7, 'OrbB', 7.5, A)
...
def draw_puppeteer_hloc(c, char):
    ...
    filled_rect(c, 0, hloc_y_bot, PW, hloc_h, HexColor('#13132a'))
    ...
    for i,(loc,m_rng,r_rng,loc_hp) in enumerate(pup_locs):
        ry -= row_h
        bg = HexColor('#1a1a3a') if i%2==0 else HexColor('#121228')
